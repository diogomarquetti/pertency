...
/**
 * Sobe a foto pro bucket "estudantes-fotos" e atualiza estudantes.foto_url
 * direto do client — mesmo padrão de components/usuarios/foto-card.tsx. Só
 * existe em modo edição, porque precisa do estudanteId.
 */
export function FotoCard(props: FotoCardProps) {
  ...
  function handleFile(file: File | null) {
    ...
    const { estudanteId, escolaId } = props;
    ...
    startTransition(async () => {
      ...
      await removeArquivosExistentes(supabase, escolaId, estudanteId);
  ...
  return (
    <Card className="gap-4 p-[24px]">
      <h2 className="flex items-baseline gap-2 text-highlight text-ink">
        Foto <span className="text-[12.5px] font-medium text-muted">(opcional)</span>
      </h2>
